    ...
    public class DatabaseService
    {
        ...
        public void InitializeDatabase()
        {
            ...
            using (var connection = new LiteDatabase(filePath))
            {
                ...
                if (result == 0)
                {
                    GenerateFakeData(numberOfEmployees, numberOfExpenses, connection);
                }
            }
        }

        private void GenerateFakeData(int numberOfEmployees, int numberOfExpenses, LiteDatabase connection)
        {
            var employees = connection.GetCollection<Employee>();
            var expenses = connection.GetCollection<Expense>();
